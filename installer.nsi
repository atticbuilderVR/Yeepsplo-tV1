...
; Pages
  ...
  ; Custom Page: Admin User
  Page custom nsDialogsUserPage nsDialogsUserPageLeave
  ; Custom Page: Theme Selection
  Page custom nsDialogsThemePage nsDialogsThemePageLeave
...
Section "Dashboard Core" SecCore
  ...
  WriteRegStr HKCU "Software\DashboardApp" "AdminUser" $AdminUser
  WriteRegStr HKCU "Software\DashboardApp" "Theme" $ThemeValue

  ; Register for Windows Add/Remove Programs (Settings)
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\DashboardApp" "DisplayName" "Yeepsploit Dashboard"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\DashboardApp" "UninstallString" "$\"$INSTDIR\Uninstall.exe$\""
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\DashboardApp" "DisplayIcon" "$\"$INSTDIR\dashboard.exe$\""
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\DashboardApp" "Publisher" "Yeepsploit"
...
Section "Start Menu Shortcuts" SecShortcuts
  ...
  ; Uninstall shortcut removed to force usage of Windows Settings
SectionEnd
...
Section "Uninstall"
  ...
  RMDir "$INSTDIR"
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\DashboardApp"
...
Function nsDialogsUserPage
  ...
  !insertmacro MUI_HEADER_TEXT "Admin Setup" "Set up the main administrator account."

  ${NSD_CreateLabel} 0 0 100% 12u "Enter the Admin Username:"
  Pop $Label

  ${NSD_CreateText} 0 13u 100% 12u "admin"
  Pop $AdminUserInput

  nsDialogs::Show
FunctionEnd

Function nsDialogsUserPageLeave
  ${NSD_GetText} $AdminUserInput $AdminUser
  ${If} $AdminUser == ""
    MessageBox MB_OK|MB_ICONSTOP "Username cannot be empty."
    Abort
  ${EndIf}
FunctionEnd

Function nsDialogsThemePage
  nsDialogs::Create 1018
  Pop $Dialog
  !insertmacro MUI_HEADER_TEXT "Appearance" "Select the dashboard theme."
  ${NSD_CreateLabel} 0 0 100% 12u "Choose your preferred theme:"
  Pop $Label
  ${NSD_CreateRadioButton} 0 20u 100% 10u "Dark Mode"
  Pop $RadioDark
  ${NSD_CreateRadioButton} 0 35u 100% 10u "Light Mode"
  Pop $RadioLight
  SendMessage $RadioDark ${BM_SETCHECK} ${BST_CHECKED} 0
  nsDialogs::Show
FunctionEnd

Function nsDialogsThemePageLeave
  ${NSD_GetState} $RadioDark $0
  ${If} $0 == ${BST_CHECKED}
    StrCpy $ThemeValue "Dark"
  ${Else}
    StrCpy $ThemeValue "Light"
  ${EndIf}
FunctionEnd
